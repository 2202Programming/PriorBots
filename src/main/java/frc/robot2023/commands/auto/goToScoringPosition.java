...
import frc.robot2023.commands.Automation.CenterTapeYaw;
...
public class goToScoringPosition extends Command {
  ...
  public void initialize() {
    ...
    if(DriverStation.getAlliance() == DriverStation.Alliance.Blue) { //BLUE ALLIANCE
      ...
      targetPose = frc.base.FieldPoses.blueScorePoses[scoringBlock][scoringSubstation];
    }
    else { //RED ALLIANCE
      ...
      targetPose = frc.base.FieldPoses.redScorePoses[scoringBlock][scoringSubstation];
    }
    pathCommand = MoveToPoseAutobuilder(constraints, targetPose);
    sdt.disableVisionPose();
    RobotContainerOrig.RC().limelight.enableLED();
    RobotContainerOrig.RC().limelight.setPipeline(1); //no more april tag update during the move, get ready for tape
  ...
  public void end(boolean interrupted) {
    pathCommand.cancel();
    sdt.stop();
    RobotContainerOrig.RC().limelight.disableLED();
    RobotContainerOrig.RC().limelight.setPipeline(0); //back to apriltag pipe
