...
public class autoCommand extends Command {
  ...
  public autoCommand() {
    this.drivetrain = RobotContainerOrig.RC().drivetrain;
  }
  ...
  public void initialize() {
    ...
    SequentialCommandGroup tempCommand = new SequentialCommandGroup(
      RobotContainerOrig.RC().autoBuilder.fullAuto(pathGroup.get(0)),
      new outtakeCompetitionToggle().withTimeout(2));
      
      if (dc.readSideboard(SBButton.Sw12)){
        tempCommand.addCommands(RobotContainerOrig.RC().autoBuilder.fullAuto(pathGroup.get(1)));
      }
