...
package frc.robot2023.commands.swerve;
...
public class AllianceAwareGyroReset extends InstantCommand {
  ...
  public void initialize() {
    if (DriverStation.getAlliance() == Alliance.Blue){
      RobotContainerOrig.RC().drivetrain.resetAnglePose(Rotation2d.fromDegrees(0)); //away from blue driverstation is 0 degrees
    }
    else{
      RobotContainerOrig.RC().drivetrain.resetAnglePose(Rotation2d.fromDegrees(180)); //away from red driverstation is 180 degrees
    }
    if(disableVisionPoseRotation){
      RobotContainerOrig.RC().drivetrain.disableVisionPoseRotation();
    }
    else {
      RobotContainerOrig.RC().drivetrain.enableVisionPoseRotation();
    }
  }
}
